...
import { Inscription } from 'src/app/shared/interfaces/business.interface';
import { BusinessService } from 'src/app/core/services/business.service';

@Component({
  selector: 'app-mgn-inscriptions',
  templateUrl: './mgn-inscriptions.component.html',
  styleUrls: ['./mgn-inscriptions.component.scss'],
...
export class MgnInscriptionsComponent extends BaseListNonPagedComponent<Inscription> implements OnInit {
  ...
  override fetchData() {
    return this.businessSrv.getListInscriptions(this.searchTerm).pipe(
        catchError(err => {
  ...
  protected override filterData(items: Inscription[], filter: string): Inscription[] {
    const lowerTerm = filter.toLowerCase();
    return items.filter(inscription =>
        inscription.reference?.toLowerCase()?.includes(lowerTerm) ||
        inscription.fullName?.toLowerCase()?.includes(lowerTerm) ||
        inscription.status?.toLowerCase()?.includes(lowerTerm) ||
        inscription.fullNameConjoint?.toLowerCase()?.includes(lowerTerm)
    );
  }
