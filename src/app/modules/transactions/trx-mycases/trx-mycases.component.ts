...
@Component({
  selector: 'app-trx-mycases',
  templateUrl: './trx-mycases.component.html',
  styleUrls: ['./trx-mycases.component.scss'],
...
export class TrxMyCasesComponent extends BaseListComponent<Transaction> implements OnInit {
  ...
  loadMyTasks() {
    this.factorySrv.getTasks().subscribe({
      next: res => {
        this.tasks$ = res;
      },
      error: err => this.commonSrv.errorHandle(err, 'transactions.get_service_failed', 'transactions.service')
  ...
  searchTasks() {
    const businessKey = this.filterForm.get('businessKey')?.value;
    console.log('businessKey =', businessKey);
    this.factorySrv.searchTasks(businessKey).subscribe({
      next: res => {
        this.tasks$ = res;
      },
      error: err => this.commonSrv.errorHandle(err, 'transactions.get_service_failed', 'transactions.service')
